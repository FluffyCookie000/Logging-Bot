...
class guildBanRemove(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        print("guildBanRemove initialized")
    ...
    async def on_member_unban(self, guild, user):
        ...
        for loggingchannel in loggingchannels['channels']:
            ...
            if guild.id == channel.guild.id and permconfig['config']['guildBanRemove'] == 1:
                entry = list(await guild.audit_logs(limit=1).flatten())[0]   
                if str(entry.action) == "AuditLogAction.unban" and entry.target.id == user.id:
                    perpetratorid = entry.user.id
                    reason = entry.reason
                embed = discord.Embed(description=f'{user} was unbanned')
...
def setup(bot):
    bot.add_cog(guildBanRemove(bot))